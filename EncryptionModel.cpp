...
bool Encryption::fill_rand_data( unsigned char *fill_buffer, unsigned int fill_data )
{
    unsigned int *p = NULL;
...
void Encryption::encryption_model(std::string prototxtFile, std::string caffemodelFile, std::string outputfile)
{
    /* 读文件 校验CRC32 */
    ...
    fprototxt.open(prototxtFile, std::ios::in );
    fmodel.open(caffemodelFile, std::ios::in| std::ios::binary);
...
bool Encryption::decrypt_model(std::string encryption_model, std::string &prototxt_string, std::string &model_string)
{
    std::ifstream in;
    ...
    if(prototxt_Format.file_type_flag == 1)
    {
        ...
        decryp_buffer((unsigned char*)prototxt_file_buffer, enc_prototxt_file_size );
    ...
    if(caffemodel_Format.file_type_flag == 2)
    {
        ...
        decryp_buffer((unsigned char*)caffemodel_file_buffer, enc_caffemodel_file_size );
...
void Encryption::load_model(std::string prototxt_string, std::string model_string, boost::shared_ptr<caffe::Net<float>> &_net)
{
	...
	UpgradeNetAsNeeded("a", &_prototxt);
	...
	UpgradeNetAsNeeded("b", &_prototxt);
	_net.reset(new caffe::Net<float>(_prototxt)); 
    _net->CopyTrainedLayersFrom(_model);
